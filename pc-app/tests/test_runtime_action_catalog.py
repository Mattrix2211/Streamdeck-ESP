...
from streamdeck_companion.core.property_inspector import PropertyInspector
...
class RuntimeActionCatalogTests(unittest.TestCase):
    def test_catalog_contains_current_legacy_action_types(self) -> None:
        registry = build_runtime_action_registry()
        self.assertEqual(
            {item.id for item in registry.list()},
            {"keys", "launch", "url", "media", "audio_output", "app_volume", "app_mute", "home_assistant", "ha_adjust"},
        )
    ...
    def test_action_library_and_property_inspector_are_derived_from_same_registry(self) -> None:
        ...
        fields = inspector.fields_for("launch")
        self.assertEqual(fields[0].name, "target")
        self.assertEqual(fields[0].field_type, "path")
